...
fun EngineView.takeScreenshot(): Bitmap {
    return with(geckoView) {
        buildDrawingCache()
        val outBitmap = Bitmap.createBitmap(geckoView.drawingCache)
        destroyDrawingCache()
        outBitmap
    }
}
...
private class EngineViewExtension(engineView: EngineView) {
     ...
     */
    val geckoView: GeckoView = (engineView.asView() as FrameLayout).getChildAt(0) as GeckoView
}
