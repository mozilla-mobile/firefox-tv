...
data class ValidatedSessionParams(val url: String, val source: Session.Source)
...
object IntentValidator {
     ...
     */
    fun validateOnCreate(context: Context, intent: SafeIntent, savedInstanceState: Bundle?): ValidatedSessionParams? {
    ...
    fun validate(context: Context, intent: SafeIntent): ValidatedSessionParams? {
        ...
        when (action) {
            Intent.ACTION_MAIN -> {
                ...
                if (dialParams.isNotEmpty()) {
                    ...
                    return ValidatedSessionParams(url = "https://www.youtube.com/tv?$dialParams", source = Session.Source.ACTION_VIEW)
                }
            }
            Intent.ACTION_VIEW -> {
                ...
                return ValidatedSessionParams(dataString, Session.Source.ACTION_VIEW)
            }
            Intent.ACTION_SEND -> {
                ...
                return ValidatedSessionParams(url, Session.Source.ACTION_SEND)
            }
        }
        return null
    }
